...
class SettingsFileHelper {
    ...
    /**
     * Change this version if the SETTINGS_FILE_CONTENT changes.
     */
    private static final String VERSION = '3'
    private static final String SETTINGS_FILE_CONTENT = """${SECTION_MARKER}${VERSION} BEGIN
...
${SECTION_MARKER}${VERSION} END"""
    private static final String BEGIN_PATTERN = "${SECTION_MARKER}[0-9]+ BEGIN"
    private static final String END_PATTERN = "${SECTION_MARKER}[0-9]+ END"
    ...
    public static Collection<String> writeSettingsFile(
        ...
        if (settingsFile.exists()) {
            ...
            if (lines.find { it.contains(SETTINGS_FILE_METHOD_1_MARKER) }) {
    ...
    private static void appendSettingsFileContent(File settingsFile) {
        settingsFile.withWriterAppend { BufferedWriter bw ->
            bw.withPrintWriter { PrintWriter w ->
                ...
                // line endings right.
                for (String line in SETTINGS_FILE_CONTENT.readLines()) {
                    w.println(line)
                }
            }
        }
        ...
        for (String line in lines) {
            if (!replacing) {
                ...
                if (line ==~ END_PATTERN) {
                    ...
                    newLines.addAll(SETTINGS_FILE_CONTENT.readLines())
                    replacing = false
                    replaced = true
                } else {
