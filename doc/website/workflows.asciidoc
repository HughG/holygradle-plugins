...
[graphviz,modules_1.svg,dot,svg]
--
digraph {
    rankdir=RL;
    node [shape=box];
...
The following are some important terms from the link:glossary.html[] which you will need to understand to read this document.
...
Gradle and Artifactory use the term _artifact_ to refer to files published as part of a gloss:module[].
...
The Ivy metadata stored in Artifactory for each gloss:module[] lists a number of sets of
...
[graphviz,config_deps_1.svg,dot,svg]
--
digraph {
    rankdir=RL;
    node [shape=box];
...
The Ivy metadata stored in Artifactory for each gloss:module[] lists a number of dependencies, which
