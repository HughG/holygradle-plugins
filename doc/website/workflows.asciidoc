...
TODO: http://localwiki/Gradle#What_is_it.3F (GradleConfigurations.ppt)
...
The Gradle +name+ part of the module is fixed to be the name of the Gradle project, which itself is
fixed to be the name of the folder containing the +build.gradle+ file.  So, if you intend to really
...
link:plugin-intrepid.html#publishPackages[] block.  Note that it can be any string, not necessarily
...
<<promotion_and_republishing>>, and <<_repository_clean_up>> for more details.
...
related modules between repositories, as described in <<promotion_and_republishing>>.  You may want
...
same folder.
. Add +dependencySettings.defaultFailOnVersionConflict = false+ to your +build.gradle+.  This will
...
gloss:artifactory[] (see <<promotion_and_republishing>>), or run the script again with another repo
URL, for example,
...
As discussed under <<determine_outputs>>, the link:plugin-intrepid.html#packageArtifacts[] block
...
publish to gloss:artifactory[].  See <<check_publishing>> for how to set this with the
gloss:holygradle[].
...
<<promotion_and_republishing>> for information on how to do this.
...
To publish to gloss:artifactory[], just run +gw publish+ as described under <<check_publishing>>.
...
TIP: For the above to work, your +publishPackages+ block must come _before_ your +packageArtifacts+
block, otherwise the +_group_+ and +_version_+ will not be set correctly.
