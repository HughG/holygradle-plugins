...
For example, a module +a+ using the link:plugin-intrepid.html#_pre_defined_configuration_set_types[]
would divide its dependencies for compile-time use into +import_common+, +import_Win32_Release+, and
+import_x64_Release+, with +import_Win32_Release+ and +import_x64_Release+ extending from +import_common+.
That means that any other module which declares a dependency on +a+'s +import_x64_Release+ will pick up

* the artifacts from +import_x64_Release+ in +a+;
* the artifacts from +import_common+ in +a+;
* any transitive dependencies from +import_x64_Release+ in +a+; and
* any transitive dependencies from +import_common+ in +a+.
...
digraph {
    ...
    subgraph {
        ...
        subgraph cluster_other {
            ...
            other_build[label="build",color=blue];
        ...
        subgraph cluster_a {
            ...
            cluster_a_source -> {
                ...
                a_import_common [label="import_common",color=blue];
                a_import_Win32_Release [label="import\n_Win32\n_Release"];
                a_import_x64_Release [label="import\n_x64\n_Release",color=blue];
                a_runtime_Win32_Release [label="runtime\n_Win32\n_Release"];
                a_runtime_x64_Release [label="runtime\n_x64\n_Release"];

            } -> cluster_a_sink [style=invis,minlen=0];

            a_import_Win32_Release -> a_import_common [arrowhead="onormal"];
            a_import_x64_Release -> a_import_common [arrowhead="onormal",color=blue];
        ...
        subgraph cluster_b {
            ...
            cluster_b_source -> {
                ...
                b_import_common [label="import_common",color=blue];
                b_import_Win32_Release [label="import\n_Win32\n_Release"];
                b_import_x64_Release [label="import\n_x64\n_Release",color=blue];
                b_runtime_Win32_Release [label="runtime\n_Win32\n_Release"];
                b_runtime_x64_Release [label="runtime\n_x64\n_Release"];

            } -> cluster_b_sink [style=invis,minlen=0];

            b_import_Win32_Release -> b_import_common [arrowhead="onormal"];
            b_import_x64_Release -> b_import_common [arrowhead="onormal",color=blue];
    ...
    subgraph cluster_util {
        ...
        util_import_common [label="import_common",color=blue];
    }

    cluster_a_sink -> cluster_b_source [style=invis];

    a_import_common -> util_import_common [color=blue];

    a_import_Win32_Release -> b_import_Win32_Release;
    a_import_x64_Release -> b_import_x64_Release [color=blue];
    a_runtime_Win32_Release -> b_runtime_Win32_Release;
    a_runtime_x64_Release -> b_runtime_x64_Release;

    other_build -> a_import_x64_Release [color=blue];
    other_runtime -> a_runtime_x64_Release;
}
--
...
configuration, or dependencies on parts of a module (such as C++ headers) which are to be
encapsulated by the depending module.

Private configurations are declared in Gradle by setting the property +visible = false+ on a
configuration, and appear in the +ivy.xml+ file marked with the attribute +visibility="private"+.
...
configurations of the +other+ module are fetched.  Private configurations are shown with dashed
lines.
...
digraph {
    ...
    subgraph cluster_other {
        ...
        other_build [label="build",color=blue,style="dashed"];
    ...
    a_import_Win32_Release -> b_import_Win32_Release;
    a_import_x64_Release -> b_import_x64_Release [color=blue];
    a_runtime_Win32_Release -> b_runtime_Win32_Release [color=blue];
    a_runtime_x64_Release -> b_runtime_x64_Release [color=blue];

    other_build -> a_import_x64_Release [color=blue];
    other_runtime -> a_runtime_x64_Release [color=blue];
}
--
...
* use a single private configuration (for example, +build+) for all your dependencies;
...
Use of Gradle with C\++ was limited when the +holygradle+ plugins were created, so no single
standard set of configurations exists.  However, from version link:release_notes.html#_7_7_0[]
there are several link:plugin-intrepid.html#_pre_defined_configuration_set_types[] which will
...
exactly.  If you can use one of the link:plugin-intrepid.html#_pre_defined_configuration_set_types[]
it will make your build script much simpler.  See link:plugin-intrepid.html#_configurationSetTypes[]
and link:plugin-intrepid.html#_configurationSets[] for more information.

TIP: Declaring the dependency mapping correctly is important: if you get it wrong, you may find
...
* Boost is a build-time dependency, and this module exposes some Boost types in its interfaces,
so the comiple-time imports from Boost need to be re-exported.
* HTML Help Workshop is only used at (documentation) build time.
* ImageLib is an in-house, static library and this project doesn't expose any ImageLib types.
However, ImageLib has a runtime dependency (on the Intel C\++ compiler runtime), so this project
has to declare that its runtime configurations depend on that.  In this case, ImageLib
doesn't separate its runtime dependencies into +Debug+ and +Release+, which may either be an
oversight, or because there is only one version of the Intel runtime DLLs.  ImageLib also
does not use the pre-defined configuration names, so more complex mapping is needed.
...
At this point, you should be able to have Gradle fetch, unpack, and link all your packed
...
into a +local_artifacts+ folder or ZIP file.  This can be shared by FTP, for example.  A new
...
+local_artifacts+ folder in the same place as the project's +build.gradle+.  (The folder can also
be in any folder above the project root, to share it between multiple projects.)  The developer
can then run +gw fAD+ and all dependencies will be unpacked to the gloss:gradle[] cache, with
links added at the correct locations in the project folder.  From version
link:release_notes.html#_7_5_0[] of the Holy Gradle, the +local_artifacts+ contains a +build_info+
...
To make a +local_artifacts+ ZIP, just run 
...
* Unzip the +local_artifacts+ ZIP (or copy the +local_artifacts+ folder) into that new working
copy.
...
Library Paths it uses match the directory gloss:link[]s for the dependencies.  You may find Visual
Studio's Property Sheets useful for this, as they allow common settings to be included in multiple
projects.
...
----
gw generateDescriptorFileForIvyPublication
----
...
[TIP]
====
You can use a +file:+ URL as the target to test publishing on your own machine.  For example,
...
If you do this, you should either (a) give each publication a new version number; or (b) delete the
contents of the unpack cache each time.  Otherwise Gradle will see your new publication with the same
version, but the Holy Gradle will not unpack it, because it wll have recorded the fact that the
same-named ZIP files have already been unpacked into the relevant unpack cache folder.
====

The +publish+ task depends on the +packageEverything+ and +generateDescriptorFileForIvyPublication+
tasks, so normally you only need to run this one command after your solution is built.
...
+intrepid+ plugin will no longer know about it, and so will leave a stale link when you use it to
...
works for you, because the links still exist.  However, it may fail for other developers if they
...
Run +gw fAD+ again.  This will download and unpack any new dependencies, and create links
...
example link:files/examples/third-party/build.gradle[build script].
...
At this point, you should add +configurationSets+ and +packageArtifacts+ blocks to your script, as in
...
"+com.example-corp:my-lib+" is published at version "+1.3.1+"; then you could have multiple versions
of your release notes module, each using the date and time: "+1.3.1-_YYYYMMDD-HHMMSS_+".  Then
...
* your module's version ID (+_group_/_name_/_version_+); and
...
(You can use the +zipDependencies+ task to do this, if you also configure it to add the other files
and folders above apart from +local_artifacts/+.  To see how to do this, read the Gradle
