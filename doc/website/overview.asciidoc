...
unzipped files appear at that location in your gloss:workspace[] using gloss:symlink[]s.  The
...
link:first_steps.html#minimal_build_file[] shows how you include these plugins in your build file.
...
More detail on each plugin can be found on the link:plugins.html[] page.
