...
.gloss:artifactory[] and gloss:gradle[] in context
...
gloss:gradle[] understands that B is also needed, and will check that the version of C is
consistent.
...
gloss:artifactory[] web UI, the contents can be seen on the "Artifacts" tab.  (Note that the web UI
does not exactly match the view used by tools like gloss:gradle[], as described below.)
...
+_myrepo_-local+, and both gloss:gradle[] and the web UI refer to them in this way.
...
gloss:gradle[] they are referenced as just +_myrepo_+, and any attempt to GET a file which is not
