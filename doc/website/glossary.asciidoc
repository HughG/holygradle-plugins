...
[[gloss-configuration]] configuration::
    ...
    other parts.  Each configuration in a module has its own gloss:dependencies[], and its own
...
[[gloss-directory-junction]] directory junction::
    A directory junction is a type of NTFS reparse point that acts as an alias to a directory.
    Unlike symbolic links, a directory junction can not point to a file, only a directory, but
    be created by non-administrator users on Windows versions 8 and greater. Deleting a directory
	junction does not delete the target directory.  See also gloss:hard-link[], 
	gloss:directory-junction[].
...
configurations({ ConfigurationHandler c ->
    c.privateBuild; // creates new Configuration and assigns it to new property "privateBuild"
});
----
...
[[gloss-hard-link]] hard link::
    ...
    gloss:symbolic-link[], gloss:directory-junction[].
...
[[gloss-symbolic-link]] symbolic link::
    ...
    to create  symbolic links.  See also gloss:hard-link[], gloss:directory-junction[].
[[gloss-link]] link::
    Parts of the gloss:holygradle[] will create either a gloss:symbolic-link[] or a 
	gloss:directory-junction[] where one or the other is more appropriate.
