...
The value for this property is a Java `File` object, not a `String`.  You can create one using

* +new File(_<some String>_)+ or +new File(_<existing File>_, _<some String>_)+, in which case you must
specify the complete path; or
* +project.file(_<some String>_)+ (or just +file(_<some String>_)+), in which case the string is
relative to the project's +projectDir+ property.
...
tests {
    foo {
        ...
        standardOutput = new File(projectDir, "foo/test_results/foo_vc10_u<f>64.xml")
        // or standardOutput = new File("${projectDir}/foo/test_results/foo_vc10_u<f>64.xml")
        // or setStandardOutput file("foo/test_results/foo_vc10_u<f>64.xml")
    }
}
----
...
tests {
    foo {
        commandLine "foo/build/<Flavour>/foo_test.exe"
        commandLine "--output_format=XML"
        standardOutputTee = file("foo/test_results/foo_vc10_u<f>64.xml")
    }
}
----
