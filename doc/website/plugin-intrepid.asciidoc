...
* build tools;
* `import_*_*` in other modules, if the interface of that module should _not_ be exposed from this module.
...
* The `runtime_*_*` and `debugging_*_*` configurations in a dependency are always mapped from the corresponding
...
expected to use the `import_*_*` configurations (`import_Win32_Debug` and so on), and those configurations will
include everything in the other module's `import_common`, because each `import_*_*` configuration extends
`import_common`.

* The `import_*_*` configurations in a dependency may be
...
The rule for the `import_*_*` configurations can also be represented by the following table.  The From/To columns
...
[width="75%",cols="1,1,1,2",options="header"]
|====
|From |To |`export:` |Map to `import_*_*`?
