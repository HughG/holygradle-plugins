...
configurations in the dependency module. These mappings will be included in the module descriptor when this module is
published. The "to" configurations also determine which packaged artifacts from the dependency module will be fetched
and unpacked.
...
This container allows you to override the handling of packed dependencies to instead point to 
existing source on a local disk. This is intended for use by developers in order to test local 
changes to dependencies and projects with source overrides defined can not be published.
...
sourceOverrides {
    "framework" {
		...
		sourceOverride "../source/framework-1.1"
    }
	"ext_lib" {
		dependency "com.company.rendering:RenderingLib:2012a2"
		sourceOverride "C:/projects/renderingLib"
	}
}
...
This declares one dependency +framework+ and instead of using the packed binaries, will create 
symlinks to +../source/framework-1.1+. In addition, any transitive dependencies on +RenderingLib+ 
will be symlinked to +C:/projects/renderingLib+ instead of the packed binaries.

==== Transitive Dependencies

The transitive dependencies of a source override may differ from those of the binary package it is 
replacing. The +sourceOverride+ handler expects to find a valid Ivy file with the list of immediate
dependencies at +<sourceOverride>/build/publications/ivy/ivy.xml+ and optionally an XML file at 
+<sourceOverride>/AllDependencies.xml+ containing a full list of immediate and transitive 
dependencies for the project in the format created by the summariseAllDependencies task e.g.
...
<Configurations>
  <Configuration name="compileVc10Debug">
    <Dependency name="framework" group="com.company" version="1.0" configuration="compileVc10Debug" isSource="false" absolutePath=""/>
    <Dependency name="external-lib" group="com.company" version="1.0" configuration="compileVc10Debug" isSource="false" absolutePath=""/>
  </Configuration>
  <Configuration name="compileVc10Release">
    <Dependency name="framework" group="com.company" version="1.0" configuration="compileVc10Release" isSource="false" absolutePath=""/>
    <Dependency name="external-lib" group="com.company" version="1.0" configuration="compileVc10Debug" isSource="false" absolutePath=""/>
  </Configuration>
...
To make sure these files are up-to-date, when trying to resolve dependencies the plugin will first
check for a custom generator function declared in +build.gradle+. If none exists it will then 
attempt to run +<sourceOverride>/generateSourceOverrideDetails.bat+. If this is also not available
it will fall back to +<sourceOverride>/gw.bat generateIvyModuleDescriptor summariseAllDependencies".
If you want to skip the generation of fresh dependency files you can pass the option 
+useCachedIvyFiles+ to +gw.bat+.

==== dependency

This method specifies the coordinates of the dependency to override. It should follow the same 
pattern as the +packedDependency+ definition. Transitive dependencies of this and any source
dependencies will also be overriden but dependencies of a source override will not. This will be 
reported as a version conflict at dependency resolution time.

==== sourceOverride

This method defines a path, either absolute or relative to the root project, where the source 
replacement is located.

==== ivyFileGenerator

This method defines a closure to run to generate the dependency files for this source override. It 
should return two +File+ objects, one for the direct dependency Ivy file and the other for the 
transitive dependency XML file. The +SourceOverrideHandler+ is passed into the closure giving access
to (among other things) the project object.

This is useful in cases where the source is not a Gradle project or requires passing non-standard 
commands. This example will generate the dependency files for a sub-project of framework.
...
sourceOverrides {
	"subproject" {
		...
		sourceOverride "C:/projects/framework/subproject"
		ivyFileGenerator { SourceOverrideHandler handler ->
			project.exec {
				workingDir "C:/projects/framework"
				executable "gw.bat"
				args "subproject:summariseAllDependencies", "subproject:generateIvyModuleDescriptor"
			}
			return [
				new File("C:/projects/framework", "subproject/build/publications/ivy/ivy.xml"),
				new File("C:/projects/framework", "subproject/AllDependencies.xml")
			]
		}
	}
}
...
This example just returns static files.
...
sourceOverrides {
	"framework" {
		dependency "com.company:framework:1.1"
		sourceOverride "C:/projects/framework/"
		ivyFileGenerator {
			return [
				new File("C:/projects/framework", "ivy.xml"),
				new File("C:/projects/framework", "AllDependencies.xml")
			]
		}
	}
}
...
Dependencies that use +unpackToCache = false+ can not be overriden.
...
This task resolves all dependencies and saves them as a flat list grouped by configuration to +AllDependencies.xml+ in the project directory. For example.
...
<Configurations>
	<Configuration name="compileVc10Debug">
		<Dependency name="framework" group="com.company" version="1.0" configuration="compileVc10Debug" isSource="false" absolutePath=""/>
		<Dependency name="external-lib" group="com.company" version="1.0" configuration="compileVc10Debug" isSource="true" absolutePath="C:/projects/external-lib"/>
	</Configuration>
	<Configuration name="compileVc10Release">
		<Dependency name="framework" group="com.company" version="1.0" configuration="compileVc10Release" isSource="false" absolutePath=""/>
		<Dependency name="external-lib" group="com.company" version="1.0" configuration="compileVc10Debug" isSource="true" absolutePath="C:/projects/external-lib"/>
