...
* +RenderingLib+ packages belonging to the configurations +compileVc10X64Debug+ and +compileVc10X64Release+ should be unpacked to the central cache, and a link should be created at +<workspace>/my-rendering-lib+. When this module is published, the +RenderingLib+ dependency will be included in the module descriptor.
...
Calling this method on a packed dependency causes the +rebuildLinks+ and +deleteLinks+ task to ignore it.  This allows you to create your own folder or link in the appropriate location, to replace a packed dependency with, for example, a locally-built version.  It is the complement of the +usePublishedVersion+ property for source dependencies.
...
This task retrieves all <<_packedDependencies>> and <<_sourceDependencies>> and sets up the necessary links. Links are always deleted and rebuilt.
