...
Since version 7.0.0, the gloss:holygradle[] custom wrapper requires that the environment variable
+HOLY_GRADLE_REPOSITORY_BASE_URL+ is set to this same, site-specific URL (for example,
+http://artifact-server.example-corp.com/artifactory/+).  You should set this in
your user environment settings.  You should not need to change it unless your server name changes,
or you are working at a different site.
...
machine, so it can download the standard gloss:gradle[] distribution.  If you do not, you may see an
...
which holds the downloaded distributions of gloss:gradle[].
====

=== Running gloss:gradle[]
...
Or run this to see the default list of tasks for gloss:gradle[].
...
=== A Minimal gloss:holygradle[] Build File
...
Future versions of this page will include instructions for editing gloss:gradle[] files using
