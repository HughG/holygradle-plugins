...
This page describes how to use gloss:artifactory[Artifactory] and the gloss:holygradle[Holy Gradle] for the first time.
...
The Holy Gradle is a set of plugins for the gloss:gradle[Gradle] tool, which is a Java application.  They have only been tested on the following platform.
...
Although Gradle can be installed system-wide, usually you will add some "bootstrap" files to your project, known as a gloss:gradle-wrapper[Gradle Wrapper].  The wrapper includes a batch file which will download a Gradle distribution if necessary, then run it using that download location, so you do not need to modify your path.  The wrapper files should be committed to the source control repository for each project which uses Gradle.
