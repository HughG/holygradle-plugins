...
For versions of the Holy Gradle wrapper before 7.5.0, you must manually set up an environment variable pointing to an
organisation-specific server where the plugins can be downloaded.  (From 7.5.0 this can be committed with your project's
source code -- see link:first_steps.html#_setting_up_a_gradle_wrapper[] for details.)
