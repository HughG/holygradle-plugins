...
** You can now choose to send test output either just to a file (using +standardOutput = <File>+), or to both a file and
standard output (using +standardOutputTee = <File>+).  These methods replace +redirectOutputFilePath+ which is
deprecated (but still works, for now).  Note that these new methods don't take strings, only File objects, and they are
not automatically interpreted relative to the +projectDir+.  See link:plugin-unit-test.html#_setstandardoutput[] and
link:plugin-unit-test.html#_setstandardoutputtee[] for details.
